var graphModule = function(updateYear, resetYear, notInRange) {
  ...
  function drawDashboard() {
    ...
    function stateChangeHandler() {
      ...
      var range = YearSlider.getState();
      if (value.length > 3) {
        if ((Number(value) >= range.lowValue)&&(Number(value) <= range.highValue)) {
          updateYear(value);
        } else {
          notInRange();
          YearFilter.setState({value: ''});
        }
      } else resetYear();
    }
    function attachTextToGenre(genre) {
      return 'Number of \b'+genre+'\b songs in this year \'Top Billboards\''
    }
